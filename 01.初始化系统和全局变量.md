...
+ stage-lab-api：172.16.1.202
+ stage-mod：10.10.1.101
+ lab-mod：10.10.1.103
...
``` bash
hostnamectl set-hostname stage-lab-api # 将 stage-lab-api 替换为当前主机名
```
...
``` bash
cat >> /etc/hosts <<EOF
172.16.1.202 stage-lab-api
10.10.1.101 stage-mod
10.10.1.103 lab-mod
EOF
```
...
本操作只需要在 stage-lab-api 节点上进行，设置 root 账户可以无密码登录**所有节点**：

``` bash
ssh-keygen -t rsa 
ssh-copy-id root@stage-lab-api
ssh-copy-id root@stage-mod
ssh-copy-id root@lab-mod
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -P20022 environment.sh root@${node_ip}:/opt/k8s/bin/
...
3. 或者，安装修复了该问题的 Docker 18.09.1 及以上的版本。但由于 kubelet 也会设置 kmem（它 vendor 了 runc），所以需要重新编译 kubelet 并指定 GOFLAGS="-tags=nokmem"；
  ``` bash
  git clone --branch 1.16.6 --single-branch --depth 1 https://github.com/kubernetes/kubernetes
  cd kubernetes
